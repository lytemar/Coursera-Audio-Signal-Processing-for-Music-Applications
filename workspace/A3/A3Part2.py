...
def optimalZeropad(x, fs, f):
	...
	T = float(fs) / f					# Samples per period
	M = x.size						# Number of samples in the signal
	periodsInSample = M/T					# Number of periods in the sample
	zeroPadPeriodsNeeded = np.ceil(periodsInSample) - periodsInSample
	zeroPadSamplesNeeded = int(zeroPadPeriodsNeeded*T)
	N = M + zeroPadSamplesNeeded
	inputSignal = np.empty(N)
	inputSignal[:M] = x[:]
	inputSignal[M:] = 1e-16
	X = fft(inputSignal)
